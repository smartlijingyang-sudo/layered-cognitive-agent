"""act_safe_boundary_node — evaluate act.safe-boundary control slot.

Single-purpose worker that delegates to LcaControlActSafeBoundaryProvider.
Reads in_args, emits {allowed: bool} as a FACT.
"""

from __future__ import annotations

from agent_lab.nodes.base import Node
from agent_lab.nodes.manifest import (
    NodeKind,
    NodeLayer,
    PortInfo,
    PortKind,
    node,
)
from agent_lab.primitives.artifact import Artifact


@node(
    id="act_safe_boundary_node",
    name="act_safe_boundary_node",
    layer=NodeLayer.CONTROL,
    kind=NodeKind.VALIDATOR,
    description=(
        "Evaluate the act.safe-boundary control slot. Last-chance "
        "physical isolation gate — wraps a callable."
    ),
    inputs=[PortInfo("in_args", kind=PortKind.ARTIFACT, required=False)],
    outputs=[PortInfo("allowed", kind=PortKind.FACT)],
    provides=["act_safe_boundary_slot"],
    requires=[],
    relates_to=[],
)
class ActSafeBoundaryNode(Node):
    """Bridge to LcaControlActSafeBoundaryProvider."""

    name = "act_safe_boundary_node"

    def execute(self, node, inputs):
        from agent_lab.adapters.lca_control_act import LcaControlActSafeBoundaryProvider

        provider = LcaControlActSafeBoundaryProvider.from_node_config(node.config)
        out_port = node.config.get("to", "allowed")
        result = provider.evaluate(inputs.get("in_args"))
        return {out_port: result.get(out_port, Artifact(kind="fact", content={"allowed": True}))}
