...
@node(
    ...
    outputs=[PortInfo("routed", kind=PortKind.ARTIFACT)],
...
class RouteOn(Node):
    ...
    def execute(self, node, inputs):
        ...
        key = (
            str(key_a.content)
            if not isinstance(key_a.content, dict)
            else str(key_a.content.get("verdict", ""))
        )
        ...
        return {
            out_port: chosen.model_copy(update={"content": {**chosen.content, "routed_via": key}})
        }
