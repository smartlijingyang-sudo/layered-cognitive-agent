"""Shared helpers for control adapters (Decision dict ⇄ LCA Decision).

Think phase logic lives in ``agent_lab.nodes.think.*`` — no think adapter.
This module only keeps conversion helpers that ``lca_control`` still imports.
"""
