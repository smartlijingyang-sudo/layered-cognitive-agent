"""Adapter: agent_lab control-slot nodes ↔ LCA control slot callables.

LCA contracts consumed (read-only):
  - lca.contracts.protocols.gate.budget_policy.BudgetPolicy

agent_lab provides (this file):
  - LcaControlPerceiveContextProvider     — perceive.context slot
  - LcaControlActAuthorizeProvider        — act.authorize slot
  - LcaControlActBudgetProvider           — act.budget slot
  - LcaControlActConstrainProvider        — act.constrain slot
  - LcaControlActExecuteProvider          — act.execute slot
  - LcaControlActSafeBoundaryProvider     — act.safe-boundary slot
  - LcaControlObserveWildcardProvider     — observe.* wildcard slot
    (cross-cutting; matches LCA's ObserveWildcardExecutor no-op owner)

Each provider wraps a callable or protocol and returns ``{allowed: bool}``
(or ``{verdict: allow, event_type, ts}`` for the wildcard observer)
via an Artifact. Fixture registries let tests inject custom callables
without touching the real LCA runtime.

All fallback classes return allowed=True (identity / always-accept);
the wildcard observer's fallback always returns verdict=allow.
"""
