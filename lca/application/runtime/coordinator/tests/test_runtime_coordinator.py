...
async def test_watchdog_publishes_synthetic_terminal_if_session_terminal_but_no_spine_event(manager, clean_run_id):
    ...
    history = await manager.read_history(clean_run_id, count=10)
    assert any(e["type"] == "agent_runtime_end" and e["data"]["reason"] == "completed" for e in history)
...
async def test_watchdog_no_op_if_already_published(manager, clean_run_id):
    ...
    await coord.terminal(clean_run_id, status="done", final_state={"status": "done"})
    before_count = len([e for e in await manager.read_history(clean_run_id, count=100) if e["type"] == "agent_runtime_end"])
    # Watchdog fires
    session = MagicMock()
    session.status = "done"
    session.error = None
    session.final_state = {"status": "done"}
    await coord.synthesize_terminal_if_pending(clean_run_id, session=session)
    after_count = len([e for e in await manager.read_history(clean_run_id, count=100) if e["type"] == "agent_runtime_end"])
