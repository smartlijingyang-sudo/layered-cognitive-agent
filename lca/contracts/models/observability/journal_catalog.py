...
JOURNAL_CATALOG_META: dict[str, JournalSchemaMeta] = {
    ...
    "GateDecided": JournalSchemaMeta("required", "auditor", "internal"),
    "InboxFollowupCreated": JournalSchemaMeta("required", "auditor", "internal"),
    "TeamMessagePublished": JournalSchemaMeta("required", "auditor", "internal"),
    "ApprovalRequested": JournalSchemaMeta("required", "auditor", "internal"),
    "ApprovalResolved": JournalSchemaMeta("required", "auditor", "internal"),
    "MemoryCommitted": JournalSchemaMeta("best_effort", "operator", "internal"),
    "ContextCompacted": JournalSchemaMeta("best_effort", "operator", "internal"),
    "RunPaused": JournalSchemaMeta("required", "auditor", "internal"),
    "RunResumed": JournalSchemaMeta("required", "auditor", "internal"),
}
...
JOURNAL_CATALOG: dict[str, VocabDef] = {
    ...
    "ToolStarted": _journal(
        VocabDomain.RESOURCE,
        "lca.layer1_cognitive.body.tool_journal_emit",
    ...
    "ContextManifested": _journal(
        VocabDomain.EVENT,
        "lca.layer1_cognitive.brain.context_manifest",
        required=("digest",),
        desc="PerceiveHub 一次性发出当 step 的 ContextManifest",
    ),
    "PerceptionMerged": _journal(
        VocabDomain.EVENT,
        "lca.layer1_cognitive.perceive_hub",
        required=("delta_ref",),
        desc="Hub fold 终态",
    ),
    "GateDecided": _journal(
        VocabDomain.EVENT,
        "lca.layer1_cognitive.brain.decision_gates",
        required=("gate", "verdict"),
        desc="DecisionGate 裁决（warn/rewrite/deny）",
    ),
    "InboxFollowupCreated": _journal(
        VocabDomain.EVENT,
        "lca.harness.session",
        required=("inbox_id",),
        desc="用户输入经 Inbox 注入",
    ),
    "TeamMessagePublished": _journal(
        VocabDomain.EVENT,
        "lca.layer3_agent.team_handle",
        required=("team_id", "thread_id"),
        desc="Team 消息发布（每 Team 一个 topic）",
    ),
    "ApprovalRequested": _journal(
        VocabDomain.EVENT,
        "lca.layer1_cognitive.body.safe_executor",
        required=("envelope_id", "tool_name"),
        desc="执行信封触发审批",
    ),
    "ApprovalResolved": _journal(
        VocabDomain.EVENT,
        "lca.layer1_cognitive.body.safe_executor",
        required=("envelope_id",),
        desc="审批决议",
    ),
    "MemoryCommitted": _journal(
        VocabDomain.EVENT,
        "lca.layer1_cognitive.memory.simple_memory",
        required=("layer", "record_id"),
        desc="记忆提交",
    ),
    "ContextCompacted": _journal(
        VocabDomain.EVENT,
        "lca.layer1_cognitive.memory.simple_memory",
        required=("step",),
        desc="影子 CompactionPolicy 应用",
    ),
    "RunPaused": _journal(
        VocabDomain.EVENT,
        "lca.layer2_runtime.runtime_loop",
