...
- :mod:`lca.plugins.transport.webserver.carrier.runs.lifecycle`
  — :class:`RunLifecycleCoordinator.execute` / ``resume``
- :mod:`lca.plugins.transport.webserver.carrier.runs.execute.execution_environment`
  — :class:`RunExecutionEnvironment.prepare` (binds :class:`RunAmbit`)
- :mod:`lca.plugins.transport.webserver.carrier.runs.execute.scheduling`
  — :func:`schedule_run` (asyncio task handle)
- :mod:`lca.plugins.transport.webserver.carrier.runs.execute.environment_bindings`
  — bindings / driver / provider resolution
- :mod:`lca.plugins.transport.webserver.carrier.runs.execute.loop_drivers`
...
async def execute_run(
    ...
    *,
    ...
    import lca.plugins.transport.webserver.carrier.runs.execute as _self_pkg
...
async def resume_run(session: RunSession, registry: RunRegistry, answer: str) -> None:
    """Backwards-compat alias — durable gate + lifecycle resume."""
    from lca.plugins.transport.webserver.carrier.runs.resume import validate_durable_resume

    validate_durable_resume(session)
    # Late-bound so test patches on the execute package take effect at call time.
    import lca.plugins.transport.webserver.carrier.runs.execute as _self_pkg
