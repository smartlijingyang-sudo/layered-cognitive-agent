...
from lca.plugins.transport.webserver.handlers.runs.terminal.live_compat import LiveTail
...
def create_run_session(
    ...
    *,
    ...
    from lca.plugins.transport.webserver.handlers.runs.session.setup import RunSessionFactory
...
def _record_terminal_materialization(session: RunSession) -> None:
    ...
    Delegates to ``lca.plugins.transport.webserver.handlers.runs.terminal.materialization.record_terminal_materialization``
    (main's full impl). Kept as a private wrapper on the soft-locked
    ``gateway/runs/execute.py`` so test fixtures that call
    ``from lca.plugins.transport.webserver.handlers.runs.execute.execute import _record_terminal_materialization``
...
# lca.plugins.transport.webserver.handlers.runs.execute (a soft-lock-allowed adapter per ADR-0103 §2).
...
import lca.plugins.transport.webserver.handlers.runs.execute as _self_pkg  # noqa: E402
