"""phase.stop host — subgraph entry node for the stop phase.

Single-node subgraph host that runs the stop phase logic. The
kernel enters ``bundles/stop_subgraph.yaml`` here and dispatches this
NodeExecutor, which is the terminal node of the six-phase plan.

The standard plugin ``lca.plugins.loop.phase.stop.standard.plugin``
...
from lca.plugins.loop.phase.stop.standard.plugin import (
    StandardPhaseConfig,
    create_executor,
)
...
class StopHostExecutor:
    """Subgraph entry: pass through to the standard stop phase executor.

    Reads typed ports from upstream phases and emits a ``stop_decision``
    port so outer edges can route based on ``should_stop``.
    """

    semantic_name: str = "phase.stop.host"
    ...
    async def node_execute(
        ...
        node_id = str(context.metadata.get("node_id", "stop.main"))
        ...
        if payload is not None:
            port_values["stop_decision"] = payload
...
@plugin(
    ...
    provides=("phase:stop::phase.stop.host",),
    layer="L2",
    ...
    contract=PluginContract(
        ...
        observability=EvidenceContract(
            descriptors=("phase_stop_host.checked", "phase_stop_host.served")
        ),
    ),
    relations=(),
...
async def setup(ctx: PluginContext, config: StandardPhaseConfig) -> None:
    ctx.provide("phase:stop::phase.stop.host", StopHostExecutor())


__all__ = ["StopHostExecutor", "setup"]
