"""phase.perceive host — subgraph entry node for the perceive phase.

Single-node subgraph host that runs the perceive phase logic. The
kernel enters ``bundles/perceive_subgraph.yaml`` here and dispatches
...
The standard plugin ``lca.plugins.loop.phase.perceive.standard.plugin``
...
class PerceiveHostExecutor:
    """Subgraph entry: pass through to the standard perceive phase executor.

    Declares ``decision`` as input (carry-over from upstream observe
    frames) and emits ``observation`` so downstream ``think.main``
    can consume the typed port.
    """

    semantic_name: str = "phase.perceive.host"
    ...
    async def node_execute(
        ...
        node_id = str(context.metadata.get("node_id", "perceive.main"))
        ...
        if payload is not None:
            port_values["observation"] = payload
        return NodeOutput(port_values=port_values, next_hint=None)


@plugin(
    id="phase.perceive.host",
    Config=StandardPhaseConfig,
    provides=("phase:perceive::phase.perceive.host",),
    layer="L2",
    ...
    contract=PluginContract(
        ...
        observability=EvidenceContract(
            descriptors=("phase_perceive_host.checked", "phase_perceive_host.served")
        ),
    ),
    relations=(),
...
async def setup(ctx: PluginContext, config: StandardPhaseConfig) -> None:
    ctx.provide("phase:perceive::phase.perceive.host", PerceiveHostExecutor())


__all__ = ["PerceiveHostExecutor", "setup"]
