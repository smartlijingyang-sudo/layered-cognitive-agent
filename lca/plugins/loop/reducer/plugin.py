...
def _publish_apply_marker(
    *,
    ...
    if phase == "start":
        if emit_runtime_reducer_apply_start(method=method) is None:
            _log.debug(
                "runtime.reducer.apply marker skipped (no bound session): method=%s phase=%s",
                method,
                phase,
            )
    else:
        ...
        if emit_runtime_reducer_apply_end(method=method, outcome=outcome) is None:
            _log.debug(
                "runtime.reducer.apply marker skipped (no bound session): method=%s phase=%s",
                method,
                phase,
