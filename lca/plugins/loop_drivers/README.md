...
## 3. 输入
- 当前包内 `2` 个公开模块 + `15` 个公开符号（class / function）

## 4. 输出
- 暴露的公共 API：无 个显式 __all__ 条目； 15 个定义符号中，8 个为公共命名

## 5. 允许依赖
—

## 6. 禁止依赖
**pyproject.toml `[tool.lca.package_contracts.lca.plugins.loop_drivers].forbidden_dependencies`**:
...
- `lca/plugins/loop_drivers/registry.py`
