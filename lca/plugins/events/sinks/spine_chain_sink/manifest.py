"""spine_chain_sink Manifest（PR-5 单入口宇宙）。
...
@plugin(
    ...
    effects="none",
    description=(
        'SpineChainSink：spine.* 事实链落盘（ADR-0181 PR-9）。'
    ),
    ...
    contract=PluginContract(
        ...
        authority=AuthorityContract(grants=("event.sink",)),
        observability=EvidenceContract(descriptors=("event.spine_chain_sink",)),
    ),
    ownership=OwnershipDeclaration(
        reads=(),
        emits=("event.spine_chain_sink",),
        state_mutation="forbidden",
    ),
    marker_class=SpineChainSink,
)
async def setup(ctx: PluginContext, config: _Config) -> None:
    """spine_chain_sink boot：marker class 已在 catalog，无需实例化。"""
    return None
