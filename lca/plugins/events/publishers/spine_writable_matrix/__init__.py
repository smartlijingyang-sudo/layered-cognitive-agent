"""spine_writable_matrix — ADR-0181 PR-10 (收尾) / ADR-0183 PR-7。

writable_matrix cursor 走 spine_reflector_writable 的 typed 入口。
本 plugin 是 cursor 的 EventBus 入口骨架：旧 cursor._spine.append
路径由 EventBus.publish(SpineEventPayload, producer=WritableMatrixPlugin)
接管。

删-when：cursor 完全切到 EventBus.publish（rg
