"""spine_reflector_phase Manifest（PR-5 单入口宇宙）。
...
@plugin(
    ...
    effects="none",
    description=(
        'Phase reflectors：spine.phase.* 系列 emit（ADR-0167 PR-5）。'
    ),
    test_suite="tests/plugins/events/publishers/test_spine_reflector_phase.py",
    ...
    contract=PluginContract(
        ...
        observability=EvidenceContract(descriptors=("event.spine_reflector_phase",)),
    ),
    ownership=OwnershipDeclaration(
        reads=(),
        emits=("event.spine_reflector_phase",),
        state_mutation="forbidden",
    ),
    marker_class=ReflectorClass,
)
async def setup(ctx: PluginContext, config: _Config) -> None:
    """spine_reflector_phase boot：marker class 已在 catalog，无需实例化。"""
    return None
