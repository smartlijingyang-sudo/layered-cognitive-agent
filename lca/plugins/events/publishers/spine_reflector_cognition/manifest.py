"""spine_reflector_cognition Manifest（PR-5 单入口宇宙）。
...
@plugin(
    ...
    effects="none",
    description=(
        'Cognition reflectors：brain / critic / reasoner / prompt_assembler / synthesizer / skill_router / memory emit（ADR-0181 PR-2）。'
    ),
    ...
    contract=PluginContract(
        ...
        observability=EvidenceContract(descriptors=("event.spine_reflector_cognition",)),
    ),
    ownership=OwnershipDeclaration(
        reads=(),
        emits=("event.spine_reflector_cognition",),
        state_mutation="forbidden",
    ),
    marker_class=ReflectorClass,
)
async def setup(ctx: PluginContext, config: _Config) -> None:
    """spine_reflector_cognition boot：marker class 已在 catalog，无需实例化。"""
    return None
