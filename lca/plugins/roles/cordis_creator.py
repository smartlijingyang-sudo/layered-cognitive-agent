...
def build_cordis_creator_role_profile() -> RoleProfile:
    ...
    上层通过 ``ctx.inject("role.cordis_creator")`` 取这份数据。
    """
    return RoleProfile(
        role="cordis-creator",
        goal=PERSONA_GOAL,
...
async def setup(ctx: PluginContext, config: Config) -> None:
    """把 :func:`build_cordis_creator_role_profile` 的输出挂到 ``role.cordis_creator`` seam。"""
    ctx.provide("role.cordis_creator", build_cordis_creator_role_profile())
