...
SPEC = PluginSpec(
    ...
    provides=(CapabilityDeclaration(key="phase.edge.standard", cardinality="one", protocol="PhaseEdge", scope="profile"),),
    requires=(),
    effects=("none",),
    ...
    verification=VerificationDeclaration(test_suite="tests/declarative/test_phase_graph.py", properties=("declared_topology", "bounded_reentry")),
)
...
async def setup(ctx: PluginContext, config: Config) -> None:
    ...
    ctx.provide("phase.edge.standard", {"edges": tuple(config.edges)})
