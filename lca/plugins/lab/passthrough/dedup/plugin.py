# PR-D final 2/2 — passthrough.dedup real @plugin carrier
"""Real carrier for ``agent_lab.nodes.passthrough.dedup.plugin.Dedup``.

Mirrors the LCA ``@plugin`` decorator contract (id / provides / requires /
emits / effects / contract / setup) without importing ``cordis`` at module
top. The actual ``Dedup`` class is imported lazily inside ``setup()`` so the
cordis chain (which the legacy class transitively pulls in) is only
triggered when the carrier is actually registered with a live Cordis
Context, not when the loader walks plugin modules.

Slot id: ``lab.passthrough.dedup``
Output capability: ``(none)``

delete-when (PR-D final 2/2):
- agent_lab/nodes/passthrough/dedup/plugin.py replaced by this carrier
...
_CARRIER = LabCarrier(
    ...
    node_id='dedup',
    source_module='agent_lab.nodes.passthrough.dedup.plugin',
    source_class='Dedup',
    provides=[],
    requires=[],
    emits=[],
...
register_worker("dedup", _Dedup)
register_worker("lab.passthrough.dedup", _Dedup)
