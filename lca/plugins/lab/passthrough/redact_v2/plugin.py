# PR-D final 2/2 — passthrough.redact_v2 real @plugin carrier
"""Real carrier for ``agent_lab.nodes.passthrough.redact_v2.plugin.RedactV2``.

Mirrors the LCA ``@plugin`` decorator contract (id / provides / requires /
emits / effects / contract / setup) without importing ``cordis`` at module
top. The actual ``RedactV2`` class is imported lazily inside ``setup()`` so the
cordis chain (which the legacy class transitively pulls in) is only
triggered when the carrier is actually registered with a live Cordis
Context, not when the loader walks plugin modules.

Slot id: ``lab.passthrough.redact_v2``
Output capability: ``(none)``

delete-when (PR-D final 2/2):
- agent_lab/nodes/passthrough/redact_v2/plugin.py replaced by this carrier
...
_CARRIER = LabCarrier(
    ...
    node_id='redact_v2',
    source_module='agent_lab.nodes.passthrough.redact_v2.plugin',
    source_class='RedactV2',
    provides=[],
    requires=[],
    emits=[],
    inputs=[],
