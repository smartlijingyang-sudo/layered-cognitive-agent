...
async def setup(ctx: PluginContext, config: Config) -> None:
    ...
    # ADR-0220 §6.2 P9 splits this plugin into ``credentials`` + ``compose``;
    # P2 just removes the cordis-private ``runtime().inject("tools")`` bypass.
    # The boot-time tools list reaches ``complete_turn`` via the constructor's
    # ``tools=`` kwarg supplied by whatever wires the compose step in P9.
