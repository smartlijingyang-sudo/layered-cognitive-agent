...
# ── 内置词表 ──────────────────────────────────────────────
# 每条 ``@doc_decorator('EventName')`` 都与 ``descriptors_data.py``
...
def _doc_attach_started() -> EventDoc:
    return EventDoc(
        ...
        arch="L4 lca.plugins.transport.webserver.carrier.runs.execute;ADR-0051 run workspace plane",
        layer="L4",
    )
...
def _doc_attach_completed() -> EventDoc:
    return EventDoc(
        ...
        arch="L4 lca.plugins.transport.webserver.carrier.runs.execute",
        layer="L4",
    )
...
def _doc_attach_failed() -> EventDoc:
    return EventDoc(
        ...
        arch="L4 lca.plugins.transport.webserver.carrier.runs.execute 错误路径",
        layer="L4",
    )
