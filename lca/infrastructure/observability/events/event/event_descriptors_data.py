...
from lca.contracts.models.observability.telemetry.telemetry_catalog import VocabDomain
...
def build_default_registry() -> InMemoryEventDescriptorRegistry:
    ...
    descriptors: list[EventDescriptor] = [
        ...
        _descriptor(
            DelegationCacheHit,
            domain=VocabDomain.TEAM,
            emitter="lca.cognition.body.delegation.delegation_cache",
        ...
        _descriptor(
            SynthesisCompleted,
            domain=VocabDomain.TEAM,
            emitter="lca.cognition.body.actions.action_handlers",
            description="收口综合完成",
            durability="required",
            audience="operator",
            sensitivity="internal",
        ...
        _descriptor(
            StepCompleted,
            domain=VocabDomain.EVENT,
            emitter="lca.cognition.body.executor.simple_body",
            required=("step",),
            description="步完成",
            durability="best_effort",
            audience="operator",
        ...
        _descriptor(
            ...
            required=("tool_name",),
            description="LLM 完成一次工具调用参数的流式生成（args 收齐即发，载荷为完整 arguments）",
            durability="required",
            audience="end_user",
            sensitivity="public",
            retention="long",
        ...
        _descriptor(
            ToolStarted,
            domain=VocabDomain.RESOURCE,
            emitter="lca.cognition.body.internal.tool_journal_emit",
        ...
        _descriptor(
            ToolLifecycleEnded,
            domain=VocabDomain.RESOURCE,
            emitter="lca.harness.declarative.compile.phase.phase_execution_policy",
        ...
        _descriptor(
            ContextManifested,
            domain=VocabDomain.EVENT,
            emitter="lca.cognition.brain.pipeline.context_manifest",
            required=("digest",),
        ...
        _descriptor(
            ApprovalRequested,
            domain=VocabDomain.EVENT,
            emitter="lca.cognition.body.executor.safe_executor",
        ...
        _descriptor(
            MemoryCommitted,
            domain=VocabDomain.EVENT,
            emitter="lca.cognition.memory.simple.simple_memory",
        ...
        _descriptor(
            RunPaused,
            domain=VocabDomain.EVENT,
            emitter="lca.runtime.loop.runtime_loop",
        ...
        _descriptor(
            RunResumed,
            domain=VocabDomain.EVENT,
            emitter="lca.runtime.loop.runtime_lifecycle",
            required=("step", "reason"),
            description="Run 恢复",
            durability="required",
            audience="auditor",
        ...
        _descriptor(
            PresetPublished,
            domain=VocabDomain.EVENT,
            emitter="lca.application.authoring.preset_authoring",
        ...
        # ── ADR-0116 PR-3: boot 事件描述符 ──
        _descriptor(
            BootProfileResolved,
            domain=VocabDomain.EVENT,
            emitter="lca_kernel.source_resolve",
            required=("profile_path", "manifest_hash"),
            description="Boot K1+K2: profile 解析完成 + ResolvedProfile 哈希",
            durability="required",
            audience="operator",
            sensitivity="internal",
        ),
        _descriptor(
            BootPluginFiberSpawned,
            domain=VocabDomain.EVENT,
            emitter="lca_kernel.boot.boot",
            required=("plugin_id", "stage", "status"),
            description="Boot K3: 每个 plugin 的 Fiber spawn 开始/结束(成对 emit)",
            durability="required",
            audience="operator",
            sensitivity="internal",
        ),
        _descriptor(
            BootObservabilityAssembled,
            domain=VocabDomain.EVENT,
            emitter="lca_kernel.runtime.observability",
            required=("bound_seams", "evidence_store_kind"),
            description="Boot K5: BoundObservability 装配完成",
            durability="required",
            audience="operator",
            sensitivity="internal",
        ),
