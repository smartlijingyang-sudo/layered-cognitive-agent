...
from lca.contracts.atoms.ids.ids import RunId, TraceId, new_run_id, new_trace_id
...
def adopt_run_scope(
    *,
    ...
    (delegation, another speaker) mint a child. A resume may pass its persisted
    ``trace_id`` and source ``parent_run_id`` to retain trace-level causality
    while receiving a fresh run container. An unclaimed Gateway root keeps its
    allocated run ID but adopts the persisted trace. Returns ``(scope, is_root)``.
