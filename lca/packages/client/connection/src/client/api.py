"""Auto-generated surface skeleton for upstream ``client/connection/src/client/api.ts``.

Mirrors the public export surface of the upstream TypeScript source so that
``scripts/check_port_surface.py`` reports parity. Bodies raise
``NotImplementedError`` until a real Python implementation is filled in.

Upstream source: ``client/connection/src/client/api.ts``
"""
...
def resultOf(*args: object, **kwargs: object) -> object:
    """Surface stub for upstream function ``resultOf``."""
    raise NotImplementedError("port resultOf from client/connection/src/client/api.ts")
