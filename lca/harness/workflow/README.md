...
## 3. 输入
- 当前包内 `2` 个公开模块 + `12` 个公开符号（class / function）

## 4. 输出
- 暴露的公共 API：4 个显式 __all__ 条目； 12 个定义符号中，9 个为公共命名

## 5. 允许依赖
—

## 6. 禁止依赖
**pyproject.toml `[tool.lca.package_contracts.lca.harness.workflow].forbidden_dependencies`**:
...
- `lca/harness/workflow/engine.py`
- `lca/harness/workflow/script.py`
