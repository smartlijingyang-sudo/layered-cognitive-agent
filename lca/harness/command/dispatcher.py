...
命名宪法许可角色后缀("Gateway" 不是),语义更精确 —— session spine 的
命令路由器,负责把 HTTP-facing command 分发给 AgentRegistry / ProjectionRegistry。
4 处历史 import 与 emitter ``lca.harness.command.gateway`` wire schema 字段
通过 compat shim 兼容,过期日 2026-12-31。
"""
