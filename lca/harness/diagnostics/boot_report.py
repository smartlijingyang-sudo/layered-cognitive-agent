...
def _collect(
    ...
    # Iterate entries (preferred) or fall back to ctx-level introspection.
    ...
    for entry in raw:
        ...
        inject = _safe_tuple(getattr(entry, "inject", None))
...
def _iter_entries(ctx: Context) -> list[Any]:
    """Pull entries from a cordis Context (tolerant of shape variants)."""
    for attr in ("entries", "_entries", "plugins"):
...
def _entry_provides(entry: Any) -> Any:
    """A plugin's ``provides`` is implicit in its module's setup function.

    We can't introspect setup() without running it, so we read the
    plugin's name + id and leave provides empty unless explicitly
