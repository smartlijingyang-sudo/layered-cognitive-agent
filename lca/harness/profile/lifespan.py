...
@asynccontextmanager
async def profile_lifespan(profile_path: str | Path) -> AsyncIterator[dict[str, Any]]:
    ...
    try:
        _log.info(
            ...
            plugin_count=len(getattr(ctx, "entries", []) or []),
        )
        yield {"ctx": ctx}
    finally:
...
async def noop_lifespan(_app: _LifespanApp) -> AsyncIterator[dict[str, Any]]:
    ...
    expected to surface a 503 in that case (``gateway.runs.api._ctx_of``).
    """
    yield {}
...
def install_profile_lifespan(
    ...
    profile_path = Path(profile_path)
    return _make_profile_lifespan(profile_path)


def _make_profile_lifespan(profile_path: Path) -> Callable[[_LifespanApp], Any]:
    ...
    async def _bound_lifespan(app: _LifespanApp) -> AsyncIterator[dict[str, Any]]:
        async with starlette_profile_lifespan(app, profile_path) as state:
            yield state
