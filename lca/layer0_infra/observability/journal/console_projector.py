...
from lca.layer0_infra.observability.journal import console_render as render
from lca.layer0_infra.observability.journal.sequence_diagram import render_sequence_diagram
...
class ConsoleJournalProjector(JournalProjector):
    ...
    def on_event(self, stamped: StampedEvent) -> None:
        ...
        event = stamped.event
        if isinstance(event, TeamRunStarted):
            self._on_team_run_started(stamped, event)
        elif isinstance(event, TeamRunFinished):
            self._on_team_run_finished(stamped, event)
        elif isinstance(event, AgentRunStarted):
            self._on_agent_run_started(stamped, event)
        elif isinstance(event, AgentRunFinished):
            self._on_agent_run_finished(stamped, event)
        else:
    ...
    def _narrative_line(self, stamped: StampedEvent, event: JournalEvent) -> str:
        ...
        if isinstance(event, DelegationCacheHit):
    ...
    def _delegation_duration(self, stamped: StampedEvent, delegation_id: str) -> float | None:
        for earlier in reversed(self._state_of(stamped).events):
            event = earlier.event
            if isinstance(event, DelegationIssued) and event.delegation_id == delegation_id:
                return stamped.ts - earlier.ts
        return None
