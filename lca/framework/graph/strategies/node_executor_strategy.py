...
Node-level ``emit_on_enter`` / ``emit_on_exit`` declarations are
dispatched by :class:`~lca.framework.graph.interpreter.PlanInterpreter`
around ``strategy.execute``, not here, so they fire for every binding
(ADR-0240 / Note `2026-09-15-node-emit-dispatcher-wiring`).
"""
...
class NodeExecutorStrategy(NodeStrategy):
    ...
    async def execute(self, context: StrategyContext, input: NodeInput) -> NodeOutput:
        ...
        legacy_output: LegacyNodeOutput = await executor_call(legacy_ctx, legacy_input)
