...
_OWNER_TABLE: dict[str, list[tuple[str, str, str]]] = {
    "state_writers": [
        ...
        ("lca/layer2_runtime/", "PR-4", "runtime_loop / StopRule"),
    ...
    "control_surface": [
        ("lca/plugins/", "PR-2", "PluginDefinition.control 可选段"),
        ("bundles/", "PR-2", "Bundle YAML 显式 control"),
        ("profiles/", "PR-2", "Profile YAML 显式 control"),
    ],
}
        ...
        for f in slot_findings:
            ...
            out.append(
                Violation(
                    ...
                    col=f.col,
                    kind=f.kind,
                    message=f"{slot}: {f.message}" if slot != "__missing_control__" else f.message,
                    owner_pr=owner_pr,
                    rationale=rationale,
                )
            )
