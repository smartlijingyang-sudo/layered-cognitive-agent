...
def _ledger_high_watermark_for(path: Path) -> int:
    """取 journal.jsonl 最后一条 ``seq`` 字段作为高水位。"""
    if not path.exists():
        return 0
    last_seq = 0
    try:
...
def _terminal_event_id_for(path: Path) -> str:
    """找最后一条 ``AgentRunFinished`` 的 event_id / scope.event_id。"""
    if not path.exists():
        return ""
    last_id = ""
    try:
        with path.open("r", encoding="utf-8") as fh:
            for line in fh:
                ...
                if et == "AgentRunFinished":
                    last_id = str(row.get("event_id") or row.get("scope", {}).get("event_id") or "")
    except OSError:
        return ""
    return last_id
...
def _compose_manifest(*, journal: Path, doctor: FlatArtifact | None) -> dict[str, object]:
    ...
    terminal_event_id = _terminal_event_id_for(journal)
