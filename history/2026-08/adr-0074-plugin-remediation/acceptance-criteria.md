...
| audit 只接受声明式控制贡献 | `uv run pytest --no-cov tests/harness/test_audit_control_surface.py -v` | 声明式贡献被清点，重新引入原始 `control=` 元数据会被拒绝 |
...
```sh
uv run pytest --no-cov tests/plan/test_plan_compiler.py -v
```

预期：声明式贡献编译为唯一的 `control_entries` 投影，聚合与失败语义由 typed contribution 校验。

### 2.3 Activation DSL 闭集（L1）

| 操作符 | 验收命令 | 通过条件 |
|---|---|---|
| 原始 DSL 已退役 | `uv run pytest --no-cov tests/plan/test_plan_compiler.py -v` | 原始 `ControlPlan` / Activation DSL 不再进入运行计划；控制谓词由声明式阶段图的 typed contribution 校验 |
| 贡献形状合法 | 同上 | 非 typed phase / role / executor 的 contribution 在声明接缝失败 |
| 单一事实源 | 同上 | `CompiledRunPlan` 只保留声明式 `control_entries`，不允许并行控制子计划 |
...
通过条件：按语义阶段列出已编译的声明式 control entry（capability / predicate / aggregation / evidence）。

**停留概念红旗**：`explain` 命令存在但只 print `[]`，或只 print `"未实现"`——意味着运行时未真正消费声明式计划。

---

## 3. ADR-0068 验收：CompiledRunPlan + CommandEnvelope

### 3.1 CompiledRunPlan = CapabilityPlan + ScopePlan + 声明式控制投影（L1+L2）

| 子 plan | L1 验收 | L2 验收 |
|---|---|---|
| `CapabilityPlan` | `lca/contracts/protocols/capability_plan.py` 提供稳定能力投影与 hash | `spawn.bind_plan` 返回的 `CompiledRunPlan.capability` 字段非空、含 provider binding |
| 声明式控制投影 | `PluginSpec.contributes` 编译为 `CompiledRunPlan.control_entries` | `./scripts/lca-ops explain control <phase>` 输出已编译 entries（§2.5） |
| `ScopePlan` | `lca/contracts/protocols/scope_plan.py` 含 ScopePlan 契约 | `plan.scope` 字段含 grant / budget / lease / ExecutionSpace |
...
**停留概念红旗**：plan_ref 字段在 JournalEntry 里存在但常为 None；或 replay 时能力、作用域或声明式控制投影拿不到。
...
| **R6** | ControlPlan 在 runtime 路径上从未被读取 | `uv run pytest --no-cov tests/declarative/test_runtime_driver.py -v` | 命中必须含 `plan.control.slot_entries` 或等价调用；否则 = 装饰性新增 | ⚠️ YELLOW（声明式运行路径存在，但当前 runtime driver 有 1 项最终状态传播失败） |
...
|:-:|---|---|---|:-:|
| **V1** | 控制面单一入口（11 slot） | §2.5 `./scripts/lca-ops explain control <slot>` + `tests/declarative/test_runtime_driver.py` | 11 slot 全部 L2 接线 + explain 命令可执行 | ⚠️ YELLOW（控制面接线证据存在，但声明式 runtime 仍有结果收敛回归） |
...
**当前状态（2026-08-24）**:
- ⚠️ V1-V12：核心数据面大部分通过，但 V1/R6 受声明式 runtime 结果传播回归影响，不能记为全绿。
- ⚠️ CV1-CV6：静态架构约束大部分通过，但全量测试与协议/类型门禁未通过。
- ⚠️ §7.1/§7.2/§7.3/§7.4：局部验收通过，不能替代全量 pytest。
- ❌ R1-R12：历史迁移基线为 0，但 `check_protocol_impl.py` 和 `check_no_any.py` 仍失败。

**结论**: 当前代码已经落地 ADR-0066/0068/0069/0074 的核心数据结构和 ADR-0075/0076 的主要声明式路径，但尚未满足 §9.4 的完整 sign-off 公式。必须先修复审计报告 `ADR_66_69_74_75_76_IMPLEMENTATION_AUDIT.md` 中列出的运行时结果传播、兼容回退、替换性、Protocol 继承、类型安全和全量回归问题。
...
| 2026-08-22 | 最终核验记录曾将 ControlPlan、端到端矩阵和 R1-R12 标为 GREEN；2026-08-24 复核发现该结论超出当前可复现实证，详见审计报告。 |
...
| 2026-08-24 | 复核：将已删除的 旧版 control runtime 测试路径 引用改为 `tests/declarative/test_runtime_driver.py`，并记录当前全量与架构门禁失败；详见仓库根目录审计报告。 |
| 2026-08-26 | 控制面收敛：退役并行 `ControlPlan`、原始 `control=` 元数据和旧运行时解释器；验收命令改为验证 `PluginSpec.contributes → CompiledRunPlan.control_entries` 的唯一投影。 |
