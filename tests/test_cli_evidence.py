...
def _drive_run(jsonl_path: Path, ev_root: Path) -> tuple[str, dict, str]:
    ...
    return "run_evi", big, ts.event.state_ref.digest
...
def test_cli_evidence_no_seam_exit_2() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        sr = EvidenceRef(
            ...
            byte_length=100, classification=Classification.INTERNAL,
            retention=RetentionClass.RUN_DEFAULT, locator="",
        ).to_dict()
        from lca.contracts.models.observability.journal import StampedEvent
        from lca.layer0_infra.observability.journal.journal_io import stamped_to_record
        # Build a ToolStarted with state_ref
        scope = RunScope(trace_id="t", run_id="run_x", agent_role="r")
        tool_evt = ToolStarted(tool_name="t", invocation_id="i", state_ref=EvidenceRef.from_dict(sr))
        ...
        with _bb(bound_no_seam):
            result = _RUNNER.invoke(
                app, ["evidence", "run_x", digest, "--jsonl", str(jsonl)]
            )
...
def test_cli_evidence_human_readable_default() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with _bb(bound):
            result = _RUNNER.invoke(
                app, ["evidence", run_id, digest, "--jsonl", str(jsonl)]
            )
        assert result.exit_code == 0
