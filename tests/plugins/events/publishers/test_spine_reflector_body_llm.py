...
stream.token + stream.stall）在 Session 路径下能正常 publish + 鉴权通过。
"""
...
def _run() -> None:
    ...
    # tool execute
    ref = plugin.emit_body_tool_execute_start(tool_name="search", invocation_id="inv-1", attempt=1)
    ...
    # decision wrapper
    ref = plugin.emit_body_tool_decision_start(tool_name="search", invocation_id="inv-1")
    assert ref.category == "spine.body.tool.execute.start"
...
def test_emit_body_llm_all(bound_session: Any) -> None:
    _run()
