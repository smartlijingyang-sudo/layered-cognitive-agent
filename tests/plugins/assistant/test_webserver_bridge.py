...
from lca.plugins.assistant.webserver.bridge import (
...
class TestMountAssistantRoutes:
    def test_mount_adds_all_route_paths(self) -> None:
        from lca.plugins.assistant.webserver.bridge import _mount_assistant_routes
...
class TestRegisterCookie:
    """ADR-0252 D8：bridge.register 转发浏览器会话 Cookie。"""

    @pytest.mark.asyncio
    async def test_cookie_forwarded_in_headers(self) -> None:
        from unittest.mock import AsyncMock, patch
    ...
    @pytest.mark.asyncio
    async def test_no_cookie_omits_header(self) -> None:
        from unittest.mock import AsyncMock, patch
