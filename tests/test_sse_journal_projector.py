...
class TestSseProjectorStepTextDeltaFilter(unittest.TestCase):
    def test_decision_channel_is_not_filtered(self) -> None:
        """Filter lives in the UI door. Projectors must not drop journal events."""
        received: list[str | None] = []
        projector = SSEJournalProjector(received.append)
        scope = RunScope(trace_id="t", run_id="r")
        ...
        self.assertEqual(len(non_none), 2)
        self.assertIn("raw token", non_none[0])
        self.assertIn("visible text", non_none[1])
