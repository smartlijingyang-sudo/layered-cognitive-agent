...
class TestDeclarativeControlAudit:
    """The audit discovers native contributions and rejects retired metadata."""

    def test_audit_inventories_native_contribution_and_flags_control_kwarg(
        self, tmp_path: Path
    ) -> None:
        from lca.harness.diagnostics.audit.control_surface import scan_control_surface

        source = tmp_path / "plugin.py"
        source.write_text(
            "PhaseContribution(executor='control.test')\nplugin(control=legacy_control)\n",
            encoding="utf-8",
        )

        findings = scan_control_surface((tmp_path,))

        assert findings["control.test"][0].kind == "declarative_control_contribution"
        assert findings["__retired_control_metadata__"][0].kind == "retired_control_metadata"
