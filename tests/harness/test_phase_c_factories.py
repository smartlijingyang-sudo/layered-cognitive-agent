...
from lca.plugins.loop_cognitive import CognitiveLoopFactory
from lca.plugins.loop_replay import ReplayLoopFactory, ReplayLiveAgent
...
class TestCognitiveLoopFactory:
    def test_create_agent(self, scope: ScopedPluginHost, tmp_path: Path):
        """Test that CognitiveLoopFactory creates an agent."""
        factory = CognitiveLoopFactory()
        options = AgentOptions(max_steps=5)
        
        async def _test():
            handle = await factory.create(scope, "test-session", options)
