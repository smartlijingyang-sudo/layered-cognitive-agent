...
class _ScriptableEvidenceStore(EvidenceStore):
    ...
    def prepare(self, payload, *, classification, retention, media_type="application/octet-stream", prepared_by=""):
        return self._fs.prepare(
            payload, classification=classification, retention=retention,
            media_type=media_type, prepared_by=prepared_by,
        )
...
def _make_app_with_bound(
    bound: BoundObservability | None, jsonl_path: Path | None = None,
):
    ...
    from gateway.runs.api import get_run_evidence
    ...
    if jsonl_path is not None:
        class _StubRegistry:
            def jsonl_path_for(self, _run_id: str) -> Path:
                return jsonl_path
        application.state.registry = _StubRegistry()
    else:
...
def _drive_run_with_evidence(jsonl_path: Path, ev_root: Path) -> tuple[str, dict, dict]:
    ...
    # Find the ToolStarted and capture its state_ref
    from lca.layer0_infra.observability.journal.journal_io import read_journal

    events = read_journal(jsonl_path)
    ts = [e for e in events if isinstance(e.event, ToolStarted)][0]
    assert ts.event.state_ref is not None
    return "run_e2e", big, {
        "digest": ts.event.state_ref.digest,
        "ref": ts.event.state_ref,
    }
...
def _write_envelope_with_state_ref(jsonl_path: Path, *, digest: str, byte_length: int) -> None:
    ...
    line = {
        ...
        "occurred_at": 1.0,
        "committed_at": 1.0,
        "scope": {"trace_id": "t", "run_id": "abc", "parent_run_id": None,
                  "parent_trace_id": None, "delegation_id": None,
                  "agent_role": "researcher", "step": 0},
...
def test_evidence_endpoint_integrity_violation_500() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        sr = EvidenceRef(
            algorithm="sha256", digest=digest, media_type="application/json",
            byte_length=100, classification=Classification.INTERNAL, retention=RetentionClass.RUN_DEFAULT, locator="",
        ).to_dict()
        line = {
            "schema": "lca.journal/2", "event_id": f"evt_{digest[:24]}",
            "run_id": "abc", "run_seq": 1, "occurred_at": 1.0, "committed_at": 1.0,
            "scope": {"trace_id": "t", "run_id": "abc", "parent_run_id": None,
                      "parent_trace_id": None, "delegation_id": None,
                      "agent_role": "researcher", "step": 0},
            "causation": {"parent_event_id": "", "links": []},
