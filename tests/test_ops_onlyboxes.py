...
from lca.layer0_infra.ops.config import OnlyboxesConfig
...
class _FakeProbe:
    ...
    def observe(self):
        from lca.layer0_infra.ops.services.onlyboxes import OnlyboxesObservation
...
def test_stack_status_includes_onlyboxes() -> None:
    ...
    assert (
        "onlyboxes"
        not in __import__("lca.layer0_infra.ops.steps", fromlist=["STOP_SERVICES"]).STOP_SERVICES
    )
