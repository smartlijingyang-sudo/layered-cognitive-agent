...
class TestTypedRelationAccessors:
    ...
    def test_invocation_scope_canonicalizes_to_turn(self) -> None:
        rel = TypedRelation(
            source="a", target="b", kind=Relation.GOVERNS, scope=Scope.INVOCATION
        )
        d = typed_relation_to_dict(rel)
...
class TestProviderBinding:
    ...
    def test_provider_bindings_from_iter(self) -> None:
        bindings = provider_bindings_from_iter(
            [
                {"capability": "memory", "owner_plugin": "p1"},
                {"capability": "tools", "owner_plugin": "p2", "effect_class": "tools"},
            ]
        )
        assert len(bindings) == 2
        assert bindings[0].capability == "memory"
        assert bindings[1].effect_class == "tools"
...
class TestCapabilityPlanAccessors:
    def test_relations_of_kind(self) -> None:
        ...
        plan = CapabilityPlan(
            profile_path="x", provider_bindings=(), relations=(r1, r2, r3)
        )
    ...
    def test_relations_to_plugin(self) -> None:
        ...
        r2 = TypedRelation(source="b", target="c", kind=Relation.EXECUTES)
        plan = CapabilityPlan(
            profile_path="x", provider_bindings=(), relations=(r1, r2)
        )
...
class TestProjectCapabilityPlan:
    ...
    def test_disabled_excluded_by_default(self) -> None:
        ...
        assert len(plan_disabled.provider_bindings) >= len(
            plan_default.provider_bindings
        )
    ...
    def test_validate_targets_rejects_unknown_source(self) -> None:
        ...
        try:
            ...
            with pytest.raises(
                CapabilityPlanResolveError, match="not in known plugin"
            ):
    ...
    def test_validate_targets_rejects_bad_target_for_fact_relation(self) -> None:
        ...
        try:
            ...
            with pytest.raises(
                CapabilityPlanResolveError, match="fact descriptor"
            ):
                project_capability_plan(resolved)
        finally:
            plugin.definition.setup.meta = original_meta
    ...
    def test_non_list_relations_rejected(self) -> None:
        ...
        try:
            ...
            with pytest.raises(
                CapabilityPlanResolveError, match="must be list/tuple"
            ):
                project_capability_plan(resolved)
        finally:
            plugin.definition.setup.meta = original_meta
    ...
    def test_non_mapping_relation_rejected(self) -> None:
        ...
        try:
            ...
            with pytest.raises(
                CapabilityPlanResolveError, match="must be mapping"
            ):
                project_capability_plan(resolved)
        finally:
            plugin.definition.setup.meta = original_meta
