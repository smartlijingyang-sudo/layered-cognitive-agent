"""ADR-0169 D8 / PR-25:ObservabilityRuntime 缝族装配测试。

Runtime 已经实现于 ``lca_kernel.observability.ObservabilityRuntime``;
本测试聚焦 PR-25 任务模板中的最小契约:
- Runtime 缝族字段都被填(cursor_factory / projection_host / persistence /
  barrier);不接受 None。
...
def _build_seam_ctx() -> Any:
    """Build a cordis Context pre-populated with the observability seam registries (PR-7)."""
...
def test_runtime_holds_seam_components(tmp_path: Path) -> None:
    """``ObservabilityRuntime`` 缝族字段都被填:cursor_factory / host / persistence / barrier(ADR-0169 D8)。"""
