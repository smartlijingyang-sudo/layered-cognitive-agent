"""Tests for ``CursorRecord`` — phase-window advance + lazy import.

Drives the real shipped ``CursorRecord`` class.  After ADR-0185 P5 the
...
3. Cursor accepts → ``cursor.advance(target)`` invoked.
"""
