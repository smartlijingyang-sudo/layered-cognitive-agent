...
钉 capability key 钉 (assistant.catalog / skill_overlay / evolve / jobs) 已落
进 ``lca.contracts.capabilities`` 登记表与 ``CAPABILITIES_BY_KEY`` 自动索引；
点分小写 + ``"assistant."`` 前缀 + cardinality=``"one"`` 全部由本测试守住
（PR-3+ 插件 ``provides`` 字段直接 import 这些常量）。
"""
...
class TestAssistantCapabilityKeys:
    ...
    def test_cardinality_is_one(self) -> None:
        """PR-2 范围内四个 capability 都是 ``"one"`` —— 拒绝并列多实现（§6 禁 God Catalog）。"""
        for cap in (
            ASSISTANT_CATALOG,
            ASSISTANT_SKILL_OVERLAY,
            ASSISTANT_EVOLVE,
