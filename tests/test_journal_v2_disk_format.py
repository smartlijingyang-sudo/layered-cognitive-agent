...
- ``event_id`` is deterministic across re-emit.
"""
...
def test_disk_v2_envelope_shape() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for line in lines:
            ...
            # event_id format
            assert line["event_id"].startswith("evt_") and len(line["event_id"]) == 28
            # scope
...
def test_disk_v2_data_strips_view_only_fields() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for line in lines:
            ...
            for forbidden in {"arguments_preview", "result_preview", "plugin_state", "output_truncated"}:
...
def test_disk_v2_event_id_is_deterministic() -> None:
    """Re-running the same run with the same seq/ts produces the same event_id."""
    with tempfile.TemporaryDirectory() as tmp:
        jsonl_a = Path(tmp) / "a.jsonl"
        jsonl_b = Path(tmp) / "b.jsonl"
        _drive_solo_run(jsonl_a)
        _drive_solo_run(jsonl_b)
        ids_a = [json.loads(x)["event_id"] for x in jsonl_a.read_text().splitlines() if x]
        ids_b = [json.loads(x)["event_id"] for x in jsonl_b.read_text().splitlines() if x]
        # event_id should match across runs (deterministic hash on seq + ts).
        # We don't enforce full equality because ts uses time.time() which
        # may differ across runs; the structural shape must hold.
        assert len(ids_a) == len(ids_b) == 7
        for ea, eb in zip(ids_a, ids_b):
            assert ea.startswith("evt_") and eb.startswith("evt_")
            assert len(ea) == len(eb) == 28
...
def test_disk_v2_read_reconstructs_stamped_events() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # event_id preserved on in-memory StampedEvent
        for e in events:
...
def test_disk_v2_descriptor_validates_payload() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        jsonl = Path(tmp) / "j.jsonl"
        _drive_solo_run(jsonl)
        for line in [json.loads(x) for x in jsonl.read_text().splitlines() if x]:
            d = line["descriptor"]
