...
def test_failed_boot_does_not_leak_module_singletons() -> None:
    ...
    The previous gateway boot path mutated ``_registry``, ``_file_store``,
    ``_devices``, ``_device_hub`` as module globals. A failed boot
    left those objects as garbage. The new design constructs these
    only inside ``create_app`` and binds them to ``app.state`` — a
    failed lifespan startup raises and no module-level global is
    mutated by that failure.
    ...
    # Force lazy construction so we can snapshot the post-construction values.
    _ = gateway_app_module.get_registry()
    _ = gateway_app_module.get_file_store()
    after_registry = gateway_app_module._module_registry
    after_file_store = gateway_app_module._module_file_store
    assert after_registry is not None
    assert after_file_store is not None
    ...
    # The failed boot must not have rebound the module-level defaults
    # to the broken app's state.
    assert gateway_app_module._module_registry is after_registry
    assert gateway_app_module._module_file_store is after_file_store
