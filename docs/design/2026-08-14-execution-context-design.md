...
npx @lca/host connect --gateway ws://127.0.0.1:8765 --token lca-local-host
```
...
npx @lca/host connect --gateway ws://127.0.0.1:8765
# → mkdir C:\Users\lichao\LCA\sandbox
# → device_id = hostname

# 远程 Windows（指定 LobeHub 用户，自动创建目录）
npx @lca/host connect --gateway ws://127.0.0.1:8765 --user bob
