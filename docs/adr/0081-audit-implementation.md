...
- Step 1: 实现 `RuntimeIdempotencyStore` 于 `lca/runtime/declarative_runtime.py`
  ...
  - 相关契约测试覆盖成功去重、失败 Observation 去重和不确定 effect
  - 当前默认 store 为进程内实现；跨重启持久化由 durable backend 负责
...
| Effect Idempotency | ✅（进程内） | `RuntimeIdempotencyStore` + `RuntimeEffectGateway` claim/complete；跨重启需 durable backend |
...
**结论**：ADR-0075 的核心目标已达成。默认生产路径完全由 `CompiledRunPlan` → `GraphAssembler` → `GenericPlanInterpreter` → `RuntimeEffectGateway` 驱动，legacy runtime 和 composer fallback 已完全移除，effect idempotency 已实现为进程内 claim/complete 协议；跨重启的 claim/receipt 持久化仍需 durable backend。工具失败与恢复边界见 [`../specs/tool-failure-recovery.md`](../specs/tool-failure-recovery.md)。Recovery profile 的完整 plugin 实现作为未来工作延迟。
