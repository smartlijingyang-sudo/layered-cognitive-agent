...
发生审批暂停时，`CognitivePhaseGraphPlan.approval_resume_node` 是唯一的恢复位置；它必须引用图中已声明的 `think` 节点。解释器把该节点写入 `PhaseRunCursor`，不拥有节点名称或 Profile 默认值。触发审批但计划未声明恢复位置时，解释器以 `PG-008` 失败结果关闭执行，避免将不可重放的默认值写入 cursor。

终止节点的 `PhaseResult.payload` 必须是 `StopDecision`。解释器仅从该类型读取 `should_stop`、状态和最终输出；字典或任意对象等未声明 payload 以 `RT-002` 失败结果关闭执行，而不得被默认为完成。
...
插件的可执行输出受限于 `PhaseResult`、`RunFact`、`RunDelta`、`ControlVerdict`、`Decision`、`CommandEnvelope`、`Observation`、`Reflection`、`WriteSet` 与 `StopDecision`。所有 State 更改仍由 Reducer 在 Journal 顺序中应用；所有世界副作用仍通过 Effect Gateway 获得 receipt。阶段事务只能调用 `DeltaReducer.apply_delta`；需要兼容其他 reducer 形状时，适配器必须在该 seam 外完成，不得在事务中按属性猜测方法。`StopDecision.status` 与 `final_output` 必须作为 `stop` delta 经 `DeltaHandler` 和 `Reducer` 折叠到 `AgentState`，再由运行时转换为 `Result`；解释器与 Effect Gateway 不得直接写入最终状态。
...
核心声明式路径已完全上线：`CompiledRunPlan` → `GraphAssembler` → `GenericPlanInterpreter` → `RuntimeEffectGateway` 为默认生产路径。Legacy runtime loop、control_policies engine、v1 composer fallback、dual-write 已从生产代码中移除（Tasks 1-6）。Effect idempotency 通过 `idempotency_store` capability 注入；生产默认绑定为 `SqliteIdempotencyStore`，跨重启保留 `in_progress` 与 completed receipt，并对不确定 effect 保持 fail-closed 语义。Recovery edge 已由原生 typed `PluginSpec` provider 声明，显式携带 predicate、bounded loop、预算、终止谓词和 evidence；完整 replay/Journal E2E 仍按 full-plugin-remediation 计划验收。

验收矩阵已通过 68+ 项测试；Ruff、Mypy、`plugin check --strict`、`plan validate` 与 `audit declarative-boundaries` 均通过。架构守护测试 `test_production_sources_do_not_reference_removed_runtime_modules()` 验证无 legacy 引用。详见 [`0081-audit-implementation.md`](0081-audit-implementation.md)。
...
### 待完成的插件化迁移（2026-08-22 审计）

尽管核心声明式路径已上线，生产代码仍存在待收敛的兼容路径与类型边界，完整整改计划见 [历史整改计划](../../history/2026-08/adr-0074-plugin-remediation/remediation-plan.md)。Effect receipt/idempotency 的跨重启 durable backend 与 bounded recovery provider 已纳入 capability graph；完整 replay/Journal E2E、全量类型门禁和其他矩阵项仍按该计划治理。

#### 🔴 P0 — 必须插件化（违反宪法/ADR 原则，阻塞真实扩展）
...
1. **Phase 1**：在 `lca/contracts/protocols/` 定义 6 个新 Protocol（DecisionClassifier / EffectHandler / DeltaHandler / ActionHandler / ArtifactClosure / GateChainComposer）
2. **Phase 2**：在 `lca/plugins/seam_definitions/` 注册对应 seam
3. **Phase 3**：在 `lca/plugins/providers/` 实现默认插件
4. **Phase 4**：改造核心 runtime 使用 seam 注入
5. **Phase 5**：给 `DefaultReducer` / `ClosedSetTopology` / `DefaultStopRule` 加 `@plugin`
6. **Phase 6**：补齐测试

#### 验收标准

- [ ] 所有 11 项完成
- [ ] `lca-ops inspect-tree` 可见所有新插件
- [ ] 可通过 Profile patch 替换所有默认实现
- [ ] 无 `Any` 类型（Phase 4 完成后）
- [ ] 全量 pytest 通过
