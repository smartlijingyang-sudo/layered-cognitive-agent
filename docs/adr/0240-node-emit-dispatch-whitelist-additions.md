...
**Implemented** (2026-09-15). Plan: `.superpowers/sdd/2026-09-15-node-emit-dispatch-wiring/PLAN.md`. Spec (Agent Note, now `implemented/`): [`../notes/implemented/seam/2026-09-15-node-emit-dispatcher-wiring.md`](../notes/implemented/seam/2026-09-15-node-emit-dispatcher-wiring.md). Validation: `tests/framework/graph/test_node_executor_strategy_emit.py` 6/6 + `tests/lca_kernel/boot/test_node_event_emission_check.py` 13/13. `NodeEventEmissionCheck` re-enable deferred to follow-up PR.
...
The 10 EP names below form the closed set of dispatchable execution points for the node-level emit wiring. Each maps to one `emit_*_for_state` helper in `lca/infrastructure/session/emit/cognitive_emit.py` (created by the implementation task) and one `_EP_DISPATCH` entry in `lca/loop/emit/node_emitter.py`. The driver calls `emit_for_node` at node enter and exit, contained with `contextlib.suppress(Exception)`.
...
|---|---|---|---|---|---|
| 1 | `terminal.commit` | `NodeExecutorStrategy` post-dispatch on outer-plan `terminal.commit` node exit | `emit_terminal_commit_for_state` | `spine.terminal.commit` category → `*.spine.jsonl` fold to `terminal_outcome` | whitelist: `spine.py:82`; category: `spine.py:259`; yaml: `spine.yaml:740`; test: `tests/loop/emit/test_node_emitter_dispatch.py::test_terminal_commit_dispatch` |
| 2 | `phase.perceive.fold` | `NodeExecutorStrategy` post-dispatch on perceive phase-graph fold node | `emit_phase_perceive_fold_for_state` | `spine.phase.perceive.fold` category → fold deriver for perceive projection | whitelist: `spine.py:71`; category: `spine.py:255`; yaml: `spine.yaml:701`; test: `tests/loop/emit/test_node_emitter_dispatch.py::test_phase_fold_dispatch` |
| 3 | `phase.think.fold` | `NodeExecutorStrategy` post-dispatch on think phase-graph fold node | `emit_phase_think_fold_for_state` | `spine.phase.think.fold` category → fold deriver for think projection | whitelist: `spine.py:72`; category: `spine.py:256`; yaml: `spine.yaml:711`; test: `tests/loop/emit/test_node_emitter_dispatch.py::test_phase_fold_dispatch` |
| 4 | `phase.reflect.fold` | `NodeExecutorStrategy` post-dispatch on reflect phase-graph fold node | `emit_phase_reflect_fold_for_state` | `spine.phase.reflect.fold` category → fold deriver for reflect projection | whitelist: `spine.py:75`; category: `spine.py:260`; yaml: `spine.yaml:760`; test: `tests/loop/emit/test_node_emitter_dispatch.py::test_phase_fold_dispatch` |
| 5 | `phase.remember.fold` | `NodeExecutorStrategy` post-dispatch on remember phase-graph fold node | `emit_phase_remember_fold_for_state` | `spine.phase.remember.fold` category → fold deriver for memory projection | whitelist: `spine.py:73`; category: `spine.py:257`; yaml: `spine.yaml:722`; test: `tests/loop/emit/test_node_emitter_dispatch.py::test_phase_fold_dispatch` |
| 6 | `phase.stop.fold` | `NodeExecutorStrategy` post-dispatch on stop phase-graph fold node (`PhaseName.stop`) | `emit_phase_stop_fold_for_state` | `spine.phase.stop.fold` category → fold deriver for stop projection | whitelist: `spine.py:74`; category: `spine.py:258`; yaml: `spine.yaml:732`; test: `tests/loop/emit/test_node_emitter_dispatch.py::test_phase_fold_dispatch` |
| 7 | `phase.act.fold.start` | `NodeExecutorStrategy` pre-dispatch on act phase-graph fold-start node | `emit_phase_act_fold_start_for_state` | `spine.phase.act.fold.start` category → fold deriver for act projection (paired with existing `phase.act.fold.end`) | whitelist: `spine.py:76`; category: `spine.py:261`; yaml: `spine.yaml:771`; test: `tests/loop/emit/test_node_emitter_dispatch.py::test_phase_fold_dispatch` |
| 8 | `think.gate.end` | `NodeExecutorStrategy` post-dispatch on `think.gate` node exit (paired with existing `think.gate.start` at line 41) | `emit_think_gate_end_for_state` | `spine.cognition.think.gate.end` category → gate-decided projection | whitelist: `spine.py:42`; category: `spine.py:194`; yaml: `spine.yaml:63`; test: `tests/loop/emit/test_node_emitter_dispatch.py::test_think_gate_end_dispatch` |
| 9 | `phase_graph.subgraph.enter` | `SubgraphStrategy` pre-dispatch on subgraph recursion entry | `emit_phase_graph_subgraph_enter_for_state` | `spine.phase_graph.subgraph.enter` category → graph projection (depth tracking) | whitelist: `spine.py:104`; category: `spine.py:281`; yaml: `spine.yaml:873`; test: `tests/loop/emit/test_node_emitter_dispatch.py::test_subgraph_enter_exit_dispatch` |
| 10 | `phase_graph.subgraph.exit` | `SubgraphStrategy` post-dispatch on subgraph recursion exit | `emit_phase_graph_subgraph_exit_for_state` | `spine.phase_graph.subgraph.exit` category → graph projection (outcome + depth) | whitelist: `spine.py:105`; category: `spine.py:282`; yaml: `spine.yaml:885`; test: `tests/loop/emit/test_node_emitter_dispatch.py::test_subgraph_enter_exit_dispatch` |
