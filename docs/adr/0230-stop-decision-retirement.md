...
**Open defect this amendment exposes.** `ToolLoopBreakerGate` (breaks at 3 same-tool failures) and `ProgressLoopDetector` (forces RESPOND at 6 no-progress steps) both read `control_turns(state)`, which only `Reducer.apply_turn` writes, from `TurnDeltaHandler` in **remember**. The failed-tool path is `act.main → think.main` and skips remember, so on that path both gates see zero turns and cannot fire. `append_turn_control_fact`, the producer of the durable `turn.control.v1` SSOT, has no callers at all. This predates the amendment and equally affects successful tool loops; it is why the budget guard is currently the only working bound. Fixing it means committing the control turn where the turn happens (`act.observe.commit_fact`) instead of in a phase the path may skip, and de-duplicating against the remember path.
...
| Body | unclassified receipt (`failure_kind is None` and `outcome == failed`) | `ERROR` | `act.observe.terminate_decide` sets `should_terminate=True` on the result payload; outer-plan edge routes to `terminal.commit`. Originally `failure_kind == execution`; see Amendment |
