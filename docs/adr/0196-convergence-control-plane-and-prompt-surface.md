...
1. **Convergence 控制面**（contracts）— `DeliveryEvidence`、`ConvergenceVerdict`
...
> **2026-09-14 修订（task-class taxonomy 退役）**：regex 把用户文本分类成 `informative_text` / `visual_artifact` / `code_demo` / `mixed` 的逻辑被证明不可靠（与"ask is what it is"的契约不符），整条 `TaskClass` 字面量、`sensor.convergence-task`、manifest hint、`predicates` 分支、`policy.nudge` 阈值、`payload.task_requires_synthesis`、`prompt/surface` 的 `informative_text` 例外一并删除。`DeliveryEvidence` 收敛到 **artifact + producer success + user-visible text** 三元谓词。CV3 撤销；MV5 删除（见 ADR-0201）。
...
| CV3 | 交付已满足时 producer 工具决策 MUST rewrite 为 RESPOND（`DeliverySatisfiedGate`） |
| CV4 | 收敛评估与 prompt 渲染 MUST 发 session catalog 事件（debug-run 可 grep） |
