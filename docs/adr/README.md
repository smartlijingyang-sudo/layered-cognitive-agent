...
| [0213](0213-kernel-serve-spawn-result-and-health-readiness.md) | KernelServe spawn 结果结构化 + /health plugin readiness 字段 — `KernelServeSpawner` 5 原子 step（preflight/start/port_bound/http_ready/plugin_ready）+ stderr 每 spawn 独立落盘 + timeout 永不为 True + `kernel-restart` 子命令拆出 `stack.heal` + `/health` body 新增 `plugin` 字段；配套 Note [`2026-09-09-kernel-serve-spawn-state-machine`](../../notes/proposed/seam/2026-09-09-kernel-serve-spawn-state-machine.md)；落地分 PR-1/2/3 | Proposed |
