...
默认实现由 `lca.plugins.seam_definitions.observability.run_ledger` 声明并由 `bundles/base.yaml` 装配；其 `RunLedgerFactory` Protocol 以单个 `run_id` 创建隔离的文件系统账本，并拒绝包含路径分隔符的 run identity。Capability manifest 检查必须递归扫描现行 `lca/plugins/seam_definitions/` 布局，并依据 `implements` 的 contracts-layer 类型证据校验声明。
