...
| ConsoleObservability | ConsoleNarratorExporter → ConsoleJournalProjector（ADR-0037：console 改由 journal 投影） |
| ConsoleNarratorExporter | ConsoleJournalProjector（span 叙述导出器被 journal 投影器取代，ADR-0037） |
| JSONLFileObservability | JsonlExporter → JsonlJournalProjector（ADR-0037：落盘改 journal schema） |
| JsonlExporter | JsonlJournalProjector（span 级 JSONL 导出器被 journal 落盘投影器取代，ADR-0037） |
