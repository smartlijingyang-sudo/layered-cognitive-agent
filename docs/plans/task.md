...
| FIX-FE-STARTUP-RESILIENCE | 根治前端启动命令缺陷：补丁自愈能力、禁止隐瞒错误、严格健康状态与启动阻断 | Completed | 1. 补丁自愈（engine.py）：reconcile() 增加 marker_broken 检测，当补丁虽记录已应用但目标文件丢失 verify_marker 时，自动从 .lobehub-upstream 还原对应文件并重置为 pending，使后续 apply() 能够在纯净 upstream 基础上自动自愈，彻底消除补丁脱落后无法自愈的盲区；2. 绝不伪造健康（lobehub.py）：重构 state()，补丁 broken 时严格返回 DEGRADED 状态并列出损坏补丁，杜绝在端口存活时显示 fake healthy；3. 启动窄门严格拦截（start() & ensure_ready()）：start() 必须执行 ensure_ready()，若补丁 broken 或存在前置缺陷立即 fail-loud 拒绝拉起并返回 STOPPED 与明确修复指令（python3 deploy/lobehub/patch_lobehub.py），彻底禁止带病启动；4. 修复 in-place 自愈（heal()）：DEGRADED 状态下触发 patch_lobehub.py 原地打补丁并刷新状态；5. 测试矩阵：test_ops_lobehub.py（13/13）与 test_lobehub_reconcile.py（11/11）全数通过，无 lint 错误 |
