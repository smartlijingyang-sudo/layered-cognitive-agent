...
- **Capability keys**: the `PhaseExecutorLookup` Protocol is intentionally narrow (`(binding, node_id, region) -> Any`). Phase plugins today resolve executors by `ctx.capabilities.get("phase.reflect.standard")` etc. The lookup must map `node_id` (e.g. `reflect.main`) to the right capability key. Mitigation: the provider plugin derives the mapping; the historical mechanism is described in `docs/design/2026-09-14-agent-lab-info-graph-reference.md` §4.4.
