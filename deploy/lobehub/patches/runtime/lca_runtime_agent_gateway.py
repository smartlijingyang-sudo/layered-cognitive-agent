...
def _patch_conversation_control(ctx: PatchContext) -> bool:
    ...
    if "assistant rows persist only on turn seal" not in control_text:
        opt_anchor = (
        ...
        if opt_anchor not in control_text:
            raise SystemExit(
                "[lca_runtime_agent_gateway] conversationControl optimistic update anchor not found"
            )
        opt_guarded = (
            "    // 1. Mark intervention as approved and set tool result to user's response\n"
            "    const toolContent = options?.toolResultContent ?? `User submitted: ${JSON.stringify(response)}`;\n"
            "    if (options?.skipResume && options.lcaRunId) {\n"
            "      // LCA gateway resume: assistant rows persist only on turn seal, so the\n"
            "      // DB snapshot echoed back by the core optimistic updates is hollow.\n"
            "      // Replacing the store with that snapshot would clobber the streamed\n"
            "      // reasoning/tools on the assistant rows. Update the tool row in memory\n"
            "      // only; the backend persists the answer via resume_tool_result.\n"
            "      // NOTE: use updateMessage with pluginIntervention (not updateMessagePlugin)\n"
            "      // because the intervention card checks msg.pluginIntervention, not msg.plugin.intervention.\n"
            "      this.#get().internal_dispatchMessage(\n"
            "        {\n"
            "          id: toolMessageId,\n"
            "          type: 'updateMessage',\n"
            "          value: { pluginIntervention: { status: 'approved' } },\n"
            "        },\n"
            "        optimisticContext,\n"
            "      );\n"
            "      this.#get().internal_dispatchMessage(\n"
            "        { id: toolMessageId, type: 'updateMessage', value: { content: toolContent } },\n"
            "        optimisticContext,\n"
            "      );\n"
            "      if (options?.pluginState) {\n"
            "        this.#get().internal_dispatchMessage(\n"
            ...
            "      }\n"
            "    } else {\n"
            "      // NOTE: use internal_dispatchMessage with updateMessage and pluginIntervention\n"
            "      // (not optimisticUpdateMessagePlugin) because the intervention card checks\n"
            "      // msg.pluginIntervention, not msg.plugin.intervention.\n"
            "      this.#get().internal_dispatchMessage(\n"
            "        { id: toolMessageId, type: 'updateMessage', value: { pluginIntervention: { status: 'approved' } } },\n"
            "        optimisticContext,\n"
            "      );\n"
            "\n"
            "      await this.#get().optimisticUpdateMessageContent(\n"
        ...
        control_text = control_text.replace(opt_anchor, opt_guarded, 1)
        ctx.write(control_path, control_text)
        changed = True
        control_text = ctx.read(control_path)
