...
class ReportGenerator:
    ...
    def _register_fonts(self):
        """注册中文字体 —— PDF 必须嵌入字体"""
        pdfmetrics.registerFont(TTFont(self.brand.font_family, self.brand.font_path))
    ...
    def generate(self, data: dict, output_path: str):
        ...
        elements.append(
            Paragraph(f"生成日期：{datetime.date.today().isoformat()}", self.styles["BrandBody"])
        )
    ...
    def _header_footer(self, canvas, doc):
        ...
        canvas.drawString(20 * mm, 10 * mm, f"第 {doc.page} 页 | 机密文件")
```
...
def generate_data_slide(prs: Presentation, title: str, metrics: list[dict]):
    """生成数据指标卡片幻灯片"""
    ...
    for i, metric in enumerate(metrics):
        ...
        txBox = slide.shapes.add_textbox(
            x + Inches(0.2), y + Inches(0.2), card_width - Inches(0.4), Inches(0.8)
        )
        tf = txBox.text_frame
        p = tf.paragraphs[0]
...
def generate_large_report(data_iterator, output_path: str, headers: list[str]):
    """
    流式生成大数据量 Excel
