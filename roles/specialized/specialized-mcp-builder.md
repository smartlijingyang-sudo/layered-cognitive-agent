...
class QueryParams(BaseModel):
    ...
    timeout_seconds: int = Field(default=30, ge=1, le=120, description="查询超时秒数")
...
async def run_query(params: QueryParams) -> list[TextContent]:
    ...
    if not sql_upper.startswith("SELECT"):
        return [
            TextContent(
                type="text", text="错误：只允许 SELECT 查询。如需修改数据，请使用对应的业务工具。"
            )
        ]
    ...
    for keyword in dangerous:
        if keyword in sql_upper:
            return [
                TextContent(
                    type="text",
                    text=f"错误：查询中包含禁止关键字 {keyword}。此工具仅支持只读查询。",
                )
            ]
