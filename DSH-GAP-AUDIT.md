...
| G8 | **Plugin-universe residual after PR-1/PR-2**: `lca/agent/orchestration_strategies/` 7 strategy classes still alive (note `2026-09-04-plugin-universe-single-entry.md` PR-3 (a) target); 7 @plugin position-escapees still outside `lca/plugins/` (`lca/runtime/reducer.py:458`, 4× `lca/cognition/team/modes/*`, `lca/application/session_live_builder_provider.py:18`, `lca_kernel/events/manifest.py` legal); 6 dead `scenario-*` / `researcher-*-tools` bundles; 4 orphan `@plugin` (null_critic / null_synthesizer / sub_composers / coding_agent_tools.py); id grammar `lca-` 148 sites still in baseline. | dsh `cordis.patch.yml` row + npm package name as the single activation truth; no `@plugin` outside `lca/plugins/` equivalent; one id grammar. | **P2** — gate hygiene; does not block DSH semantic alignment. | Yes; mechanically independent. | No (PR-3 of note is purely deletion + import update). | `ls lca/agent/orchestration_strategies/` shows 7 .py files; `rg "@plugin\b" lca/runtime/reducer.py lca/cognition/team/modes lca/application/session_live_builder_provider.py` returns 7 hits; `docs/notes/baselines/plugin-id-grammar.json` exists but id count > 0; `bundles/scenario-*.yaml` dead `$module` not removed. |
...
| 10 | **G8 — plugin-universe PR-3 (dead code) + PR-9 (7 @plugin 位置逃逸归位) + PR-11 (id grammar 滚动)** | `lca/agent/orchestration_strategies/` 7 .py alive + 7 @plugin 逃逸 + 6 dead scenario bundles are the largest **maintenance-debt** divergence from DSH `cordis.patch.yml` truth model. Bulk-delete + import update work; mechanically independent of DSH semantic alignment. | Yes | No |
