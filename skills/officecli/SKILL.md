...
# 产物目录
mkdir -p outputs
officecli create outputs/report.pptx --json
```
...
```bash
officecli create outputs/deck.pptx --json
officecli add outputs/deck.pptx / --type slide --prop title="Q4 Report" --prop background=1A1A2E --json
officecli add outputs/deck.pptx '/slide[1]' --type shape \
  --prop text="Revenue +25%" --prop x=2cm --prop y=5cm --prop font=Arial --prop size=24 --prop color=FFFFFF --json
officecli view outputs/deck.pptx outline
officecli validate outputs/deck.pptx --json
officecli view outputs/deck.pptx issues --json
```

### Word

```bash
officecli create outputs/report.docx --json
officecli add outputs/report.docx /body --type paragraph --prop text="Executive Summary" --prop style=Heading1 --json
officecli add outputs/report.docx /body --type paragraph --prop text="Revenue increased 25% YoY." --json
officecli validate outputs/report.docx --json
```

### Excel

```bash
officecli create outputs/data.xlsx --json
officecli set outputs/data.xlsx /Sheet1/A1 --prop value="Name" --prop bold=true --json
officecli set outputs/data.xlsx /Sheet1/B1 --prop value="Score" --prop bold=true --json
officecli set outputs/data.xlsx /Sheet1/A2 --prop value="Alice" --json
officecli set outputs/data.xlsx /Sheet1/B2 --prop value=95 --json
```
...
```bash
# 用户上传 → 工作根下的原文件名
officecli view input.docx outline
officecli get input.docx /body --depth 1 --json
# 改完后复制/写出到 outputs 再 export_file
officecli close input.docx --json   # 若开过 resident
cp input.docx outputs/input-edited.docx
```
...
```bash
officecli open outputs/deck.pptx --json
# … 多次 add/set …
officecli close outputs/deck.pptx --json
```
...
```bash
officecli batch outputs/deck.pptx --json <<'BATCH'
[{"op":"add","path":"/","type":"slide","props":{"title":"Q4","background":"1A1A2E"}}]
BATCH
```

交付前再 `validate`。预览走 `officecli view <file> html -o outputs/<stem>.preview.html --json`（LobeHub 原生 HTML 预览）。不要 `pip install python-pptx`。
...
```bash
# 路径错 → 先枚举子节点
officecli get outputs/deck.pptx / --depth 1 --json
officecli get outputs/deck.pptx '/slide[1]' --depth 1 --json
```
...
| 产物写 `/tmp` | 写 `outputs/` 再 `export_file` |
...
CLI 版本见 `officecli --version`。能力以 `officecli help` 为准。
执行面（沙箱镜像或本机）由运行时把 cwd 设成工作根；例子一律用相对路径。
