...
def build_activate_skill_state(
    ...
    """Skill activation card: title, description, resource flag."""
...
def build_run_command_state(
    ...
    """Command execution card: command, stdout/stderr, exit code, background flag."""
    command = first_str(args, "command") or first_str(payload, "command")
    state: dict[str, Any] = {
        "command": command,
...
def build_execute_code_state(
    ...
    """Code execution card: language, output, stderr, exit code."""
    state: dict[str, Any] = {
        "success": ok,
