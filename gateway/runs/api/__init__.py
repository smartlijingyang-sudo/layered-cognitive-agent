"""api subpackage of gateway.runs — split per ADR-0105 §11.2.

Tests use ``patch("gateway.runs.api.routes.command_endpoints.X")`` style
...
# ``gateway.runs.api.routes.command_endpoints`` resolves for mock.patch.
...
# Same exposure on the package itself for ``patch("gateway.runs.api.routes.X")``
