...
2. **8-section diagnostic.** `./scripts/lca-ops debug-run "$LATEST"` — manifest, journal summary, kernel.log tail, phase.cursor, error_ref, top stack frames, suggested_action, plan_ref + replay commands. [3/8] kernel.log is usually empty and [5/8] error_ref is a *typed label*, optionally suffixed with ` | <upstream root cause>` when the failing attempt captured provider error text. A clean [3/8] does not mean "no failure".
...
- **"debug-run says error_ref=internal"** — `error_ref` is a typed label (`node=think.main error_kind=internal attempts=1[1:permanent:ValueError]`), optionally followed by ` | <upstream root cause>` provider text; it is not the full traceback. Step 4 is mandatory; do not stop at step 2.
